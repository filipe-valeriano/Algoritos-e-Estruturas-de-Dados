...
/* Vizinhos em Comum Auxiliar - Retorna o número de vizinhos entre o vértice v1
 * e o vértice v2 */
int vizinhosEmComumAux(Grafo *g, int v1, int v2) {
  int vizinhos = 0;
  for (int y = 0; y < g->numVertices; y++) {
    if (g->matriz[v1][y] == 1 && g->matriz[v2][y] == 1) {
      vizinhos++;
    }
  }
  return vizinhos;
...
/* Vizinhos em Comum */
// Função que executa VizinhosEmComumAux pra toda linha (vértice) do grafo.
void vizinhosEmComum(Grafo *g, int v, int *vizinhos) {
  for (int index = 0; index < g->numVertices; index++) {
    vizinhos[index] = 0;
  }
  for (int x = 0; x < g->numVertices; x++) {
    vizinhos[x] = vizinhosEmComumAux(g, v, x);
  }
}

/* Coeficiente de Jaccard */
// função que divide vizinhosEmComumAux entre o vértice de número "v" e de
// número "contador" pela variável denominador, que no caso de valer 0 faz com
// que coeficientes[cont] valha 0.
void coeficienteDeJaccard(Grafo *g, int v, float *coeficientes) {
  float denominador = 0;
  for (int index = 0; index < g->numVertices; index++) {
    coeficientes[index] = 0;
  }
  for (int cont = 0; cont < g->numVertices; cont++) {
    denominador = retornaGrauDoVertice(g, v) + retornaGrauDoVertice(g, cont) -
                  vizinhosEmComumAux(g, v, cont);
    if (denominador == 0) {
      coeficientes[cont] = -1;
    } else {
      coeficientes[cont] = vizinhosEmComumAux(g, v, cont) / denominador;
    }
  }
}

/* Medida Adamic Adar */
// incrementa flag sempre que acha um denominador = 0 em uma divisão, fazendo
// com que coeficientes[x] valha -1.
void AdamicAdar(Grafo *g, int v, float *coeficientes) {
  ...
  for (int x = 0; x < g->numVertices; x++) {
    for (int y = 0; y < g->numVertices; y++) {
      if (g->matriz[x][y] == 1 && g->matriz[v][y] == 1) {
        if (log(retornaGrauDoVertice(g, y)) == 0) {
          flag++;
          break;
        }
        somatorio = somatorio + 1.0 / log(retornaGrauDoVertice(g, y));
      }
    }
    if (flag != 0)
      coeficientes[x] = -1;
...
void alocacaoDeRecursos(Grafo *g, int v, float *coeficientes) {
  ...
  for (int x = 0; x < g->numVertices; x++) {
    for (int y = 0; y < g->numVertices; y++) {
      if (g->matriz[x][y] == 1 && g->matriz[v][y] == 1) {
        somatorio = somatorio + 1.0 / retornaGrauDoVertice(g, y);
      }
    }
    coeficientes[x] = somatorio;
    somatorio = 0;
...
void similaridadeCosseno(Grafo *g, int v, float *coeficientes) {
  ...
  for (int x = 0; x < g->numVertices; x++) {
    denominador =
        sqrtf(retornaGrauDoVertice(g, x) * retornaGrauDoVertice(g, v));
    if (denominador == 0)
      coeficientes[x] = -1;
    else
      coeficientes[x] = vizinhosEmComumAux(g, x, v) / denominador;
  }
}
...
void coeficienteDeDice(Grafo *g, int v, float *coeficientes) {
  ...
  for (int x = 0; x < g->numVertices; x++) {
    denominador = retornaGrauDoVertice(g, x) + retornaGrauDoVertice(g, v);
    if (denominador == 0)
      coeficientes[x] = -1;
    else
      coeficientes[x] = 2.0 * vizinhosEmComumAux(g, x, v) / denominador;
  }
}
...
void HPI(Grafo *g, int v, float *coeficientes) {
  ...
  for (int x = 0; x < g->numVertices; x++) {
    denominador = fminf(retornaGrauDoVertice(g, v), retornaGrauDoVertice(g, x));
    if (denominador == 0)
      coeficientes[x] = -1;
    else
      coeficientes[x] = vizinhosEmComumAux(g, x, v) / denominador;
...
void HDI(Grafo *g, int v, float *coeficientes) {
  ...
  for (int x = 0; x < g->numVertices; x++) {
    denominador = fmax(retornaGrauDoVertice(g, v), retornaGrauDoVertice(g, x));
    if (denominador == 0)
      coeficientes[x] = -1;
    else
      coeficientes[x] = vizinhosEmComumAux(g, x, v) / denominador;
